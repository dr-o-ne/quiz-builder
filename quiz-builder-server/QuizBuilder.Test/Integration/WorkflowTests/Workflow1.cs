...
namespace QuizBuilder.Test.Integration.WorkflowTests {
	...
	public sealed class Workflow1 : IClassFixture<TestApplicationFactory<Startup>>, IDisposable {
		...
		public async Task Test() {
			...
			// Create Quiz 1
			(HttpStatusCode statusCode, GetQuizByIdDto data) result1 = await _httpClient.PostValueAsync<GetQuizByIdDto>( "/quizzes/", new {Name = "Quiz 1"} );
			string uid1 = result1.data.Quiz.Id;

			// Create Quiz 2
			(HttpStatusCode statusCode, GetQuizByIdDto data) result2 = await _httpClient.PostValueAsync<GetQuizByIdDto>( "/quizzes/", new {Name = "Quiz 2"} );
			string uid2 = result2.data.Quiz.Id;

			// Create Quiz 3
			(HttpStatusCode statusCode, GetQuizByIdDto data) result3 = await _httpClient.PostValueAsync<GetQuizByIdDto>( "/quizzes/", new { Name = "Quiz 3" } );
			string uid3 = result3.data.Quiz.Id;

			// Get All Quizzes
			(HttpStatusCode statusCode, GetAllQuizzesDto data) result4 = await _httpClient.GetValueAsync<GetAllQuizzesDto>( "/quizzes" );
			...
			// Update Quiz 1
			(HttpStatusCode statusCode, GetQuizByIdDto data) result5 = await _httpClient.PutValueAsync<GetQuizByIdDto>( "/quizzes/", new { Id = uid1, Name = "Quiz 1 New Name" } );

			// Update Quiz 3
			(HttpStatusCode statusCode, GetQuizByIdDto data) result6 = await _httpClient.PutValueAsync<GetQuizByIdDto>( "/quizzes/", new { Id = uid3, Name = "Quiz 3", IsVisible = true } );

			// Get All Quizzes
			(HttpStatusCode statusCode, GetAllQuizzesDto data) result7 = await _httpClient.GetValueAsync<GetAllQuizzesDto>( "/quizzes" );
			...
			(HttpStatusCode statusCode, GetAllQuizzesDto data) result9 = await _httpClient.GetValueAsync<GetAllQuizzesDto>( "/quizzes" );
			...
			//Final Check
			(HttpStatusCode statusCode, GetAllQuizzesDto data) result11 = await _httpClient.GetValueAsync<GetAllQuizzesDto>( "/quizzes" );
